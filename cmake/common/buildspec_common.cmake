...
function(_setup_obs_studio)
  ...
  elseif(OS_MACOS)
    ...
    # OBS 32's Metal renderer is Swift. Its dependency-only configure path does
    # not enable Swift, so CMake cannot infer libobs-metal's linker language.
    set(_obs_root_cmake "${dependencies_dir}/${_obs_destination}/CMakeLists.txt")
    file(READ "${_obs_root_cmake}" _obs_root_contents)
    string(
      REPLACE
      "project(obs-studio VERSION \${OBS_VERSION_CANONICAL})"
      "project(obs-studio VERSION \${OBS_VERSION_CANONICAL} LANGUAGES C CXX Swift)"
      _obs_root_contents
      "${_obs_root_contents}"
    )
    file(WRITE "${_obs_root_cmake}" "${_obs_root_contents}")
  endif()
